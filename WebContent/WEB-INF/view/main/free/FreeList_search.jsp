...
<body>
							...
							<div class="tab-content" id="nav-tabContent">
								<div class="tab-pane-board fade show active" id="nav-places" role="tabpanel" aria-labelledby="nav-places-tab">
									...
									<form action="/main/FreeList_search.do" method="post">
										...
										<a href="/main/FreeReg.do" class="btn board-btn-2 board-margin-left">글 작성하기</a>
									</form>
								</div>
							</div>
							<!-- 탭 내용 영역 종료 -->
							...
							<div class="reviewer-meta d-flex align-items-center row">
								<div class="reviewer-content col-md-9">
									<div class="review-title-ratings d-flex justify-content-between">
										<a href="/FreeDetail.do?free_seq=<%=CmmUtil.nvl(fDTO.getFree_seq()) %>"><h5><%=title%></h5></a>
									</div>
								</div>
	...
	<script>
    $(function(){
		$(".f_search").slice(0, 5).show(); // select the first ten
		$("#load").click(function(e){ // click event for load more
		    e.preventDefault();
		    $(".f_search:hidden").slice(0, 5).show(); // select next 10 hidden divs and show them
		    if($(".f_search:hidden").length == 0){ // check if any hidden divs still exist
		    }
			});
		});
    </script>
