...
public class MailService implements IMailService {
	...
	final String user = "gomn95"; // 본인 네이버 아이디
	final String password = "1027dlqeo!"; // 본인 네이버 아이디
	...
	public int doSendMail(MailDTO pDTO) {
		...
		// 전달받은 DTO로부터 데이터 가져오기(DTO객체가 메모리에 올라가지 않아 Null이 발생할 수 있기 떄문에 에러방지차원으로 if문 사용함)
		if (pDTO == null) {
			pDTO = new MailDTO();
		}
