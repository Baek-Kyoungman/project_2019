...
<body>
									...
									<div>
										...
										<div class="">
											...
											<div class="notice_box">
												<!-- Date/Time -->
												<p>
													<%=chg_dt.substring(2, 16) %> <small>by <%=CmmUtil.nvl(fDTO.getUser_id()) %></small>
												</p>
												<hr>
												<!-- Post Content -->
											...
											<div class="notice_btn_box">
												<a class="btn btn-board btn-sm" href="/mypage/dashboard_free.do">목록으로</a>
												<a href="/mypage/Mypage_free_modify.do?free_seq=<%=CmmUtil.nvl(fDTO.getFree_seq()) %>" class="btn btn-board btn-sm">수정하기</a>
												<input type="button" class="btn btn-board btn-sm pull-right" onclick="location.href='/mypage/freeDelete.do?free_seq=<%=CmmUtil.nvl(fDTO.getFree_seq()) %>'" value="삭제하기">
											</div>
										</div>
									</div>
								</div>
	...
	//alert($("input:radio[name=userCheck]:checked").val());
		...
		if (confirm("삭제 하시겠습니까?") == true) {
			document.location.href = "/mypage/freeDelete.do?free_seq=" + "<%=CmmUtil.nvl(fDTO.getFree_seq()) %>".val();
		} else {
			return false;
		}
	}
	...
	function freeModify() {
		//alert($("input:radio[name=userCheck]:checked").val());
			...
			if (confirm("수정 하시겠습니까?") == true) {
				document.location.href = "/free/freeModify.do?user_id=" + $("input[name=user_id]").val();
			} else {
				return false;
			}
		}
