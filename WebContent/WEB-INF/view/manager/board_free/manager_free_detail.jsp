...
<body>
									...
									<div>
										...
										<div class="">
											...
											<div class="notice_box">
												<!-- Date/Time -->
												<p>
													<%=chg_dt %> <small>by <%=CmmUtil.nvl(fDTO.getUser_id()) %></small>
												</p>
												<hr>
												<!-- Post Content -->
												<div id="notice_content" class="board-area" style="width: 100%">
													<p>
														<%=contents %>
													</p>
												</div>
											...
											<div class="notice_btn_box">
												<a class="btn btn-board btn-sm" href="/manager/board_free.do">목록으로</a>
												<a href="/manager/Manager_free_modify.do?free_seq=<%=CmmUtil.nvl(fDTO.getFree_seq()) %>" class="btn btn-board btn-sm">수정하기</a>
												<input type="button" class="btn btn-board btn-sm pull-right" onclick="location.href='/manager/manager_freeDelete.do?free_seq=<%=CmmUtil.nvl(fDTO.getFree_seq()) %>'" value="삭제하기">
											</div>
										</div>
									</div>
								</div>
	...
	function freeDel() {
		...
		if (confirm("삭제 하시겠습니까?") == true) {
			document.location.href = "/manager/manager_freeDelete.do?free_seq=" + "<%=CmmUtil.nvl(fDTO.getFree_seq())%>".val();
		} else {
			return false;
		}
	}
