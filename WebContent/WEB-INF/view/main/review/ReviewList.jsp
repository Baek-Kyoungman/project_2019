...
<%
	...
	int totalpg = (total - 1) / 5 + 1;

	ReviewDTO f = new ReviewDTO();
%>
					...
					<div class="hero-search-form">
						<div class="home_content_board">
							...
							<div class="tab-content" id="nav-tabContent">
								<div class="tab-pane-board fade show active" id="nav-places" role="tabpanel" aria-labelledby="nav-places-tab">
									<h6>리뷰 게시판 필터</h6>
									<form action="/main/ReviewList_search.do" method="post">
										...
										<a href="/main/ReviewReg.do" class="btn board-btn-2 board-margin-left">글 작성하기</a>
									</form>
								</div>
							</div>
							<!-- 탭 내용 영역 종료 -->
							...
							<div class="reviewer-meta d-flex align-items-center row">
								<div class="reviewer-content col-md-9">
									<div class="review-title-ratings d-flex justify-content-between">
										<a href="/ReviewDetail.do?review_seq=<%=CmmUtil.nvl(fDTO.getReview_seq()) %>"><h5><%=title%></h5></a>
									</div>
								</div>
								<div class="reviewer-name col-md-3">
									<p><%=CmmUtil.nvl(fDTO.getSidoCode()) %> / <%=CmmUtil.nvl(fDTO.getSggCode()) %> / <%=type %></p>
									<p><%=CmmUtil.nvl(fDTO.getUser_id()) %> / <%=CmmUtil.nvl(fDTO.getReg_dt().substring(2,16)) %></p>
								</div>
							</div>
						</div>
					<% } %>
					...
					<nav>
						<ul class="pagination">
							<li class="#">
								...
								%> <a href="#">&lt;</a> 
								<%} else { %> 
								<a href="/main/ReviewList.do?pgNum=<%=pgNum - 1%>">&lt;</a> 
								<% } %>
							</li>
							<li class="#">
								<%
							...
							%>
							<li class="#"><a href="/main/ReviewList.do?pgNum=<%=i%>"><%=i%></a></li>
							<% } %>
							<% } %>
							<% } else { %>
							<% if (pgNum < 4) { %>
